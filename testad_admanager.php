<?php
?>


<html>
<head>
    <meta name="viewport" content="width=device-width, initial-scale=1">

</head>
<body style='padding:0px; margin:0px;'>
<script async='async' src='https://www.googletagservices.com/tag/js/gpt.js'></script>
<script>
    var adoptagdfp = ""
    eval(atob(adoptagdfp));
</script>

<!--구글 원코드 리패리시로 변경-->
<!--
<script>
    googletag.cmd.push(function() {
        googletag.defineSlot('/223513049/ca-pub-5111137191506013-tag/primbon_testing_728x90', [[728,90]], 'div-apt-ad-WmOPxidfzq').addService(googletag.pubads());
        googletag.pubads().enableSingleRequest();
       googletag.pubads().set("page_url", "www.adop.cc");
        googletag.pubads().addEventListener('slotRenderEnded', function(event) {
            if(event.isEmpty){
                adopADshow('div-apt-ad-WmOPxidfzq',728,90,'ADOP_PBU');
            }
        });
        googletag.enableServices();
    });
</script>
<div id='div-apt-ad-WmOPxidfzq'>
    <script>
        googletag.cmd.push(function() { googletag.display('div-apt-ad-WmOPxidfzq'); });
    </script>
</div>
-->
<!----------------------------------------------------------------------------------------->
<script>
    googletag.cmd.push(function() {
        var REFRESH_KEY = 'refresh';
        var REFRESH_VALUE = 'true';
        googletag.defineSlot('/223513049/ca-pub-5111137191506013-tag/primbon_testing_728x90', [[728,90]], 'div-apt-ad-WmOPxidfzq').setTargeting(REFRESH_KEY, REFRESH_VALUE).addService(googletag.pubads());
        var SECONDS_TO_WAIT_AFTER_VIEWABILITY = 1000;
        googletag.pubads().enableSingleRequest();
        googletag.pubads().set("page_url", "www.adop.cc");
        googletag.pubads().addEventListener('impressionViewable', function(event) {
            var slot = event.slot;
            if (slot.getTargeting(REFRESH_KEY).indexOf(REFRESH_VALUE) > -1) {
                setTimeout(function() {
                    googletag.pubads().refresh([slot]);
                }, SECONDS_TO_WAIT_AFTER_VIEWABILITY * 1000);
            }
        });
        googletag.pubads().addEventListener('slotRenderEnded', function(event) {
            if(event.isEmpty){
                adopADshow('div-apt-ad-WmOPxidfzq',728,90,'ADOP_PBU');
            }
        });
        googletag.enableServices();
    });
</script>
<div id='div-apt-ad-WmOPxidfzq'>
    <script>
        googletag.cmd.push(function() { googletag.display('div-apt-ad-WmOPxidfzq'); });
    </script>
</div>


<!--<script>-->
<!--    googletag.cmd.push(function() {-->
<!--        var REFRESH_KEY = 'refresh';-->
<!--        var REFRESH_VALUE = 'true';-->
<!--        googletag.defineSlot('/5932629/ca-pub-1474238860523410-tag/inven_rt-1_250x250-1', [[250,250]], 'div-apt-ad-LvPCF5VJ3h').setTargeting(REFRESH_KEY, REFRESH_VALUE).addService(googletag.pubads());-->
<!--        var SECONDS_TO_WAIT_AFTER_VIEWABILITY = 20;-->
<!--        googletag.pubads().enableSingleRequest();-->
<!--        googletag.pubads().set("page_url", "www.adop.cc");-->
<!--        googletag.pubads().addEventListener('impressionViewable', function(event) {-->
<!--            var slot = event.slot;-->
<!--            if (slot.getTargeting(REFRESH_KEY).indexOf(REFRESH_VALUE) > -1) {-->
<!--                setTimeout(function() {-->
<!--                    googletag.pubads().refresh([slot]);-->
<!--                }, SECONDS_TO_WAIT_AFTER_VIEWABILITY * 1000);-->
<!--            }-->
<!--        });-->
<!--        googletag.pubads().addEventListener('slotRenderEnded', function(event) {-->
<!--            if(event.isEmpty){-->
<!--                adopADshow('div-apt-ad-LvPCF5VJ3h',250,250,'0faae23c-dded-4d8f-a470-ef7000f397b7');-->
<!--            }-->
<!--        });-->
<!--        googletag.enableServices();-->
<!--    });-->
<!--</script>-->
<!--<!-- /5932629/ca-pub-1474238860523410-tag/inven_rt-1_250x250-1 -->-->
<!--<div id='div-apt-ad-LvPCF5VJ3h'>-->
<!--    <script>-->
<!--        googletag.cmd.push(function() { googletag.display('div-apt-ad-LvPCF5VJ3h'); });-->
<!--    </script>-->
<!--</div>-->



<br />
<hr />
<br />
123

</body>
</html>



