...
/*전역변수 정의*/
$dbInfo     = array("127.0.0.1","root","root","adop_test"); //로컬디비
...
$krnicUrlNew = "http://krnic.or.kr/jsp/infoboard/stats/interProCurrentXml.jsp";
$krnicProxy = "http://getip.test01.com/adoptest.php";//kr닉 복사주소로컬
...
function startParsing(){
    global $krnicUrl,$fileInfo,$krnicProxy,$krnicUrlNew;
    $cnt = 0;
    
	//화일로 찍기위해 오픈
	$fp1 = fopen($fileInfo, "w") or die("Unable to open file!");
    
    //$xml = simplexml_load_string(readXml($krnicUrlNew));
    $xml = simplexml_load_string(readXml($krnicProxy));
    //var_dump($xml);exit;
    //print_r($xml->ipv4[1]);exit;
